...
/* ===== NEW IMPORT FOR TRANSPARENCY BRIDGE ===== */
import { attachContextToPrompt } from "../bridges/transparencyBridge.js";
...
`;
  ...
  async sendMessage(userMessage, acfContext = {}, options = {}) {
    ...
    let systemPrompt = acfContext.systemPrompt || this.buildSageSystemPrompt(acfContext);
    let finalUserMessage = userMessage;

    // 🟢 NEW: TRANSPARENCY BRIDGE CONTEXT ATTACHMENT
    // The calling component passes the Cognitive Field state (cfState) in acfContext.
    // We use the bridge to safely inject this context into the prompts.
    if (acfContext.cfState) {
        const { system: newSystem, user: newUser } = attachContextToPrompt({
            system: systemPrompt,
            user: userMessage,
            cfState: acfContext.cfState, // Pass the new Cognitive Field state
        });
        systemPrompt = newSystem;
        finalUserMessage = newUser;
    }
    // ----------------------------------------------
    ...
    const messages = [
      { role: 'system', content: systemPrompt },
      ...conversationHistory,
      { role: 'user', content: finalUserMessage } // Use finalUserMessage
    ];
